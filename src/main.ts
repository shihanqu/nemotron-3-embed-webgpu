import { GGUFReader, NEMOTRON3_METADATA_KEYS } from './gguf/reader.ts';
...
const DEFAULT_WEBGPU_MODEL_URL = '/models/nemotron-3-embed-1b-q4_k_m.gguf';
const DEFAULT_PREPACKED_MODEL_URL = '/prepacked-release/nemotron-3-embed-1b-q4-webgpu.wgpack';
const BASELINE_MODEL = 'text-embedding-nemotron-3-embed-1b';
const BOS_TOKEN = 1;
...
runButton.addEventListener('click', async () => {
  ...
  try {
    ...
    if (usePrepacked) {
      ...
      write(`Parsed ${prepacked.header.metadata['general.name'] ?? 'Nemotron 3 Embed 1B'}: ${prepacked.tensors.size} tensors (${compact} compact GPU matrices)`);
    } else {
      write('Fetching 715 MiB Q4_K_M GGUF fallback…');
      const response = await fetch(modelUrl);
      if (!response.ok) throw new Error(`model fetch failed: ${response.status}`);
      buffer = await response.arrayBuffer();
      model = new GGUFReader(buffer).parse({ metadataKeys: NEMOTRON3_METADATA_KEYS });
    ...
    const tokenizer = await AutoTokenizer.from_pretrained('nvidia/Nemotron-3-Embed-1B-BF16');
    if (searchParams.has('scheduler')) {
      const engine = new NemotronEmbeddingEngine(runtime, tokenizer as unknown as TokenizerLike, 16, 0, BOS_TOKEN, 11);
      ...
      for (let index = 0; index < 2; index += 1) await fetch('/baseline/v1/embeddings', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: BASELINE_MODEL, input: schedulerText }) });
      const baselineStarted = performance.now();
      for (let index = 0; index < 5; index += 1) await fetch('/baseline/v1/embeddings', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: BASELINE_MODEL, input: schedulerText }) });
    ...
    if (!acceptanceOnly) {
    const smokeText = searchParams.has('empty') ? '' : (searchParams.get('text') ?? 'The quick brown fox jumps over the lazy dog.');
    ...
    if (textTokens[0] !== BOS_TOKEN) textTokens.unshift(BOS_TOKEN);
    const sequence = textTokens.length;
    const ids = new Uint32Array(sequence);
    ids.fill(11);
    ids.set(textTokens);
    write(`Running full 16-layer graph (${textTokens.length} tokens, padded to ${sequence})…`);
    device.pushErrorScope('validation');
    ...
    if (searchParams.has('vector')) write(`VECTOR_JSON ${JSON.stringify(Array.from(embedding))}`);
    ...
    const baselineResponse = await fetch('/baseline/v1/embeddings', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: BASELINE_MODEL, input: smokeText }) });
    ...
    const baselineCosineFloor = 0.90;
    ...
    if (searchParams.has('reference')) {
      const referenceUrl = '/q40-reference/v1/embeddings';
      const q4Response = await fetch(referenceUrl, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: 'nemotron', input: smokeText }) });
      if (!q4Response.ok) throw new Error(`Q4_0 reference comparison failed: ${q4Response.status}`);
      ...
      write(`Exact Q4_0 llama.cpp agreement: ${q4Cosine.toFixed(6)}`);
    ...
    if (searchParams.has('matrix')) {
      ...
      for (const exactTokens of matrixTokenCounts) {
        ...
        if (tokens[0] !== BOS_TOKEN) tokens.unshift(BOS_TOKEN);
        ...
        const baselineResponse = await fetch('/baseline/v1/embeddings', {
          ...
          body: JSON.stringify({ model: BASELINE_MODEL, input: benchmarkText }),
        });
        ...
        if (searchParams.has('diagnostic')) write(`${exactTokens}-token LM Studio cosine: ${baselineCosine.toFixed(6)}`);
        ...
        const bytesPerRequestInLargestBuffer = exactTokens * 6144 * 2 * 2;
        const conservativeBatchCap = exactTokens >= 1500 ? 8 : 16;
        const nativeBatch = Math.max(1, Math.min(conservativeBatchCap, Math.floor(device.limits.maxStorageBufferBindingSize / bytesPerRequestInLargestBuffer)));
        const batchSizes: number[] = [];
        for (let remaining = 16; remaining > 0; remaining -= nativeBatch) batchSizes.push(Math.min(nativeBatch, remaining));
        const concurrentPlans = batchSizes.map((batch) => runtime.createPlan(batch, exactTokens));
        const concurrentBatches = batchSizes.map(makeExactBatch);
        device.pushErrorScope('validation');
        const warmBatches = await Promise.all(concurrentPlans.map((plan, index) => plan.run(concurrentBatches[index].ids, concurrentBatches[index].lengths)));
        const concurrentValidationError = await device.popErrorScope();
        if (concurrentValidationError) throw new Error(`${exactTokens}-token batch validation: ${concurrentValidationError.message}`);
        const batchCosines = warmBatches.flat().map((embedding) => reference.reduce((sum, value, index) => sum + value * embedding[index], 0));
        const worstBatchCosine = Math.min(...batchCosines);
        if (searchParams.has('diagnostic')) write(`Batch cosine diagnostics: ${batchCosines.map((value) => value.toFixed(6)).join(', ')}`);
        ...
        const concurrentStarted = performance.now();
        for (let repeat = 0; repeat < concurrentRepeats; repeat += 1) {
          await Promise.all(concurrentPlans.map((plan, chunk) => plan.run(concurrentBatches[chunk].ids, concurrentBatches[chunk].lengths)));
        }
        const aggregateRps = concurrentRepeats * 16_000 / (performance.now() - concurrentStarted);
        ...
        singlePlan.destroy();
        concurrentPlans.forEach((plan) => plan.destroy());
      }
    ...
    if (acceptanceTokens[0] !== BOS_TOKEN) acceptanceTokens.unshift(BOS_TOKEN);
    const acceptanceSequence = acceptanceTokens.length;
    const makeBatch = (batch: number) => {
      const batchIds = new Uint32Array(batch * acceptanceSequence); batchIds.fill(11);
    ...
    if (searchParams.has('sweep')) {
      ...
      for (const [workloadName, sweepText] of sweepWorkloads) {
        ...
        for (let index = 0; index < 2; index += 1) await fetch('/baseline/v1/embeddings', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: BASELINE_MODEL, input: sweepText }) });
        const baselineStarted = performance.now();
        for (let index = 0; index < 5; index += 1) await fetch('/baseline/v1/embeddings', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: BASELINE_MODEL, input: sweepText }) });
    ...
    for (let index = 0; index < 2; index += 1) await fetch('/baseline/v1/embeddings', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: BASELINE_MODEL, input: acceptanceText }) });
    const baselineRepeats = 5;
    const baselineStarted = performance.now();
    for (let index = 0; index < baselineRepeats; index += 1) await fetch('/baseline/v1/embeddings', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: BASELINE_MODEL, input: acceptanceText }) });
