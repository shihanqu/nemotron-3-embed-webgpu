...
export class NemotronEmbeddingEngine {
  ...
  embed(text: string): Promise<Float32Array> {
    ...
    if (tokens[0] !== this.bosToken) tokens.unshift(this.bosToken);
  ...
  private async flush(): Promise<void> {
    ...
    try {
      while (this.pending.length > 0) {
        const candidates = this.pending.slice(0, this.maxBatchSize);
        const candidateSequence = Math.max(...candidates.map((request) => request.tokens.length));
        const storageLimit = this.runtime.maxStorageBufferBindingSize ?? Number.POSITIVE_INFINITY;
        const bytesPerRequestInLargestBuffer = candidateSequence * 6144 * 2 * 2;
        const storageBatchCap = Math.max(1, Math.floor(storageLimit / bytesPerRequestInLargestBuffer));
        const conservativeLongContextCap = candidateSequence >= 1500 ? 8 : this.maxBatchSize;
        const requests = this.pending.splice(0, Math.min(this.maxBatchSize, storageBatchCap, conservativeLongContextCap));
        const sequence = Math.max(...requests.map((request) => request.tokens.length));
        const ids = new Uint32Array(requests.length * sequence);
        ids.fill(this.padToken);
