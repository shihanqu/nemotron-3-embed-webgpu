...
const mistralRopeShader = /* wgsl */`
enable f16;
struct Params { tokens:u32, sequence:u32, input_stride:u32, q_heads:u32, kv_heads:u32, total_pairs:u32, dispatch_width:u32 }
@group(0) @binding(0) var<storage,read> input:array<f16>;
@group(0) @binding(1) var<storage,read_write> q_output:array<f16>;
@group(0) @binding(2) var<storage,read_write> k_output:array<f16>;
@group(0) @binding(3) var<storage,read> rope_table:array<vec2<f32>>;
@group(0) @binding(4) var<uniform> params:Params;
@compute @workgroup_size(256)
fn main(@builtin(global_invocation_id) id:vec3<u32>){
  let pair_index=id.y*params.dispatch_width*256u+id.x;if(pair_index>=params.total_pairs){return;}
  let pairs_per_token=(params.q_heads+params.kv_heads)*64u;
  let token=pair_index/pairs_per_token;let head_pair=pair_index%pairs_per_token;
  let combined_head=head_pair/64u;let pair=head_pair%64u;
  let is_q=combined_head<params.q_heads;
  let head=select(combined_head-params.q_heads,combined_head,is_q);
  let heads=select(params.kv_heads,params.q_heads,is_q);
  let input_offset=select(params.q_heads*128u,0u,is_q);
  let input_base=token*params.input_stride+input_offset+head*128u+pair*2u;
  let output_base=(token*heads+head)*128u+pair*2u;
  let rope=rope_table[(token%params.sequence)*64u+pair];
  let even=f32(input[input_base]);let odd=f32(input[input_base+1u]);
  let first=f16(even*rope.x-odd*rope.y);let second=f16(even*rope.y+odd*rope.x);
  if(is_q){q_output[output_base]=first;q_output[output_base+1u]=second;}
  else{k_output[output_base]=first;k_output[output_base+1u]=second;}
}
`;

export class MistralRopeKernel {
  readonly pipeline:GPUComputePipeline;
  constructor(readonly device:GPUDevice){
    this.pipeline=device.createComputePipeline({layout:'auto',compute:{module:device.createShaderModule({code:mistralRopeShader}),entryPoint:'main'},label:'Ministral pairwise RoPE'});
  }
  createRun(input:GPUBuffer,qOutput:GPUBuffer,kOutput:GPUBuffer,tokens:number,sequence:number,ropeTable:GPUBuffer,inputStride=5120,qHeads=24,kvHeads=8):EncodableRun{
    const totalPairs=tokens*(qHeads+kvHeads)*64;
    const totalWorkgroups=Math.ceil(totalPairs/256);
    const dispatchWidth=Math.min(totalWorkgroups,this.device.limits.maxComputeWorkgroupsPerDimension);
    const params=createBufferWithData(this.device,new Uint32Array([tokens,sequence,inputStride,qHeads,kvHeads,totalPairs,dispatchWidth,0]),GPUBufferUsage.UNIFORM);
    return {pipeline:this.pipeline,bindGroup:this.device.createBindGroup({layout:this.pipeline.getBindGroupLayout(0),entries:[
      {binding:0,resource:{buffer:input}},{binding:1,resource:{buffer:qOutput}},{binding:2,resource:{buffer:kOutput}},
      {binding:3,resource:{buffer:ropeTable}},{binding:4,resource:{buffer:params}},
    ]}),dispatch:[dispatchWidth,Math.ceil(totalWorkgroups/dispatchWidth)]};
  }
}

const bidirectionalFlashAttentionShader = /* wgsl */`
enable f16;
...
@group(0) @binding(5) var<uniform> params:Params;
var<workgroup> query_tile:array<vec4<f16>,2048>;
var<workgroup> key_tile:array<vec4<f16>,512>;
var<workgroup> value_tile:array<vec4<f16>,512>;
var<workgroup> scores:array<f32,1024>;
var<workgroup> weights:array<f16,1024>;
var<workgroup> maxima:array<f32,64>;
var<workgroup> denominators:array<f32,64>;
var<workgroup> alphas:array<f16,64>;
var<workgroup> betas:array<f16,64>;
fn update_stats(row:u32){
  ...
  alphas[row]=f16(alpha);betas[row]=f16(beta);denominators[row]=denominators[row]*alpha+beta*block_den;maxima[row]=next_max;
}
fn score_value(sum:f16,q_pos:u32,k_pos:u32,valid_length:u32)->f32{
  if(q_pos>=valid_length||k_pos>=valid_length){return -3.402823e38;}
  var temperature=1.0;if(q_pos>=16384u){temperature=log(floor(f32(q_pos)/16384.0)+1.0)*0.1+1.0;}
  return f32(sum)*0.08838834764831845*temperature;
}
...
fn main(@builtin(workgroup_id) group:vec3<u32>,@builtin(local_invocation_id) local:vec3<u32>,@builtin(local_invocation_index) lane:u32){
  ...
  let valid_length=lengths[batch];
  for(var index=lane;index<2048u;index+=256u){
    let row=index/32u;let d4=index%32u;let position=group.x*64u+row;
    query_tile[index]=select(vec4<f16>(0.0),query[((batch*params.sequence+position)*params.q_heads+q_head)*32u+d4],position<valid_length);
  }
  var totals:array<vec4<f16>,8>;
  for(var slot=0u;slot<8u;slot+=1u){totals[slot]=vec4<f16>(0.0);}
  if(local.x==0u){
    for(var chunk=0u;chunk<4u;chunk+=1u){let row=local.y+chunk*16u;maxima[row]=-3.402823e38;denominators[row]=0.0;}
  }
  workgroupBarrier();
  let key_stride=select(1u,3u,valid_length>2048u);
  for(var key_base=0u;key_base<valid_length;key_base+=16u*key_stride){
    for(var index=lane;index<512u;index+=256u){
      let row=index/32u;let d4=index%32u;let k_pos=key_base+row*key_stride;
      if(k_pos<valid_length){
    ...
    workgroupBarrier();
    let k_pos=key_base+local.x*key_stride;
    var score_sums=array<f16,4>(f16(0.0),f16(0.0),f16(0.0),f16(0.0));
    for(var d4=0u;d4<32u;d4+=1u){
      let key_value=key_tile[local.x*32u+d4];
      for(var chunk=0u;chunk<4u;chunk+=1u){score_sums[chunk]+=dot(query_tile[(local.y+chunk*16u)*32u+d4],key_value);}
    }
    for(var chunk=0u;chunk<4u;chunk+=1u){
      let row=local.y+chunk*16u;let q_pos=group.x*64u+row;
      scores[row*16u+local.x]=score_value(score_sums[chunk],q_pos,k_pos,valid_length);
    }
    workgroupBarrier();
    if(local.x==0u){for(var chunk=0u;chunk<4u;chunk+=1u){update_stats(local.y+chunk*16u);}}workgroupBarrier();
    for(var slot=0u;slot<8u;slot+=1u){
      let index=lane+slot*256u;let row=index/32u;let d4=index%32u;var av=vec4<f16>(0.0);
      for(var column=0u;column<16u;column+=1u){av+=weights[row*16u+column]*value_tile[column*32u+d4];}
      totals[slot]=totals[slot]*alphas[row]+betas[row]*av;
    }
    workgroupBarrier();
  }
  for(var slot=0u;slot<8u;slot+=1u){
    let index=lane+slot*256u;let row=index/32u;let d4=index%32u;let position=group.x*64u+row;
    if(position<valid_length){output[((batch*params.sequence+position)*params.q_heads+q_head)*32u+d4]=vec4<f16>(totals[slot]/f16(denominators[row]));}
  }
}
`;

const bidirectionalSmallAttentionShader = /* wgsl */`
enable f16;
struct Params { batch:u32, sequence:u32, q_heads:u32, kv_heads:u32, value_stride:u32, value_offset:u32 }
@group(0) @binding(0) var<storage,read> query:array<vec4<f16>>;
@group(0) @binding(1) var<storage,read> key:array<vec4<f16>>;
...
var<workgroup> denominators:array<f32,16>;
...
fn main(@builtin(workgroup_id) group:vec3<u32>,@builtin(local_invocation_id) local:vec3<u32>,@builtin(local_invocation_index) lane:u32){
  let batch=group.y/params.q_heads;let q_head=group.y%params.q_heads;let kv_head=q_head/(params.q_heads/params.kv_heads);
  let valid_length=lengths[batch];let q_pos=group.x*16u+local.y;
  for(var index=lane;index<512u;index+=256u){
    let row=index/32u;let d4=index%32u;let position=group.x*16u+row;
    query_tile[index]=select(vec4<f16>(0.0),query[((batch*params.sequence+position)*params.q_heads+q_head)*32u+d4],position<valid_length);
    total_tile[index]=vec4<f16>(0.0);
  }
  if(local.x==0u){maxima[local.y]=-3.402823e38;denominators[local.y]=0.0;}
  workgroupBarrier();
  for(var key_base=0u;key_base<valid_length;key_base+=16u){
    for(var index=lane;index<512u;index+=256u){
    ...
    if(q_pos<valid_length&&k_pos<valid_length){
      var sum=f16(0.0);for(var d4=0u;d4<32u;d4+=1u){sum+=dot(query_tile[local.y*32u+d4],key_tile[local.x*32u+d4]);}
      score=f32(sum)*0.08838834764831845;
    }
    scores[local.y*16u+local.x]=score;workgroupBarrier();
    if(local.x==0u){update_stats(local.y);}workgroupBarrier();
    for(var index=lane;index<512u;index+=256u){
  ...
  for(var index=lane;index<512u;index+=256u){
    let row=index/32u;let d4=index%32u;let position=group.x*16u+row;
    if(position<valid_length){output[((batch*params.sequence+position)*params.q_heads+q_head)*32u+d4]=vec4<f16>(total_tile[index]/f16(denominators[row]));}
  }
}
`;

export class BidirectionalAttentionKernel {
  readonly pipeline:GPUComputePipeline;
  readonly smallPipeline:GPUComputePipeline;
  constructor(readonly device:GPUDevice){
    this.pipeline=device.createComputePipeline({layout:'auto',compute:{module:device.createShaderModule({code:bidirectionalFlashAttentionShader}),entryPoint:'main'},label:'bidirectional tiled flash attention'});
    this.smallPipeline=device.createComputePipeline({layout:'auto',compute:{module:device.createShaderModule({code:bidirectionalSmallAttentionShader}),entryPoint:'main'},label:'bidirectional small-context attention'});
  }
  createRun(q:GPUBuffer,k:GPUBuffer,v:GPUBuffer,output:GPUBuffer,lengths:GPUBuffer,batch:number,sequence:number,qHeads=24,kvHeads=8,valueStride=5120,valueOffset=4096):AttentionRun{
    const params=createBufferWithData(this.device,new Uint32Array([batch,sequence,qHeads,kvHeads,valueStride,valueOffset,0,0]),GPUBufferUsage.UNIFORM);
    const pipeline=sequence<=256?this.smallPipeline:this.pipeline;
    const command:EncodableRun={pipeline,bindGroup:this.device.createBindGroup({layout:pipeline.getBindGroupLayout(0),entries:[
      {binding:0,resource:{buffer:q}},{binding:1,resource:{buffer:k}},{binding:2,resource:{buffer:v}},{binding:3,resource:{buffer:output}},
      {binding:4,resource:{buffer:lengths}},{binding:5,resource:{buffer:params}},
    ]}),dispatch:[Math.ceil(sequence/(sequence<=256?16:64)),batch*qHeads]};
    return {commands:[command],ownedBuffers:[params]};
  }
...
@compute @workgroup_size(256)
fn main(@builtin(workgroup_id) group:vec3<u32>,@builtin(local_invocation_id) local:vec3<u32>){
  let batch=group.x;let lane=local.x;let length=lengths[batch];var square_sum=0.0;
  for(var d=lane;d<params.width;d+=256u){
    var total=0.0;for(var position=0u;position<length;position+=1u){total+=f32(input[(batch*params.sequence+position)*params.width+d]);}
    let mean=total/f32(length);output[batch*params.width+d]=mean;square_sum+=mean*mean;
  }
  sums[lane]=square_sum;workgroupBarrier();
  for(var stride=128u;stride>0u;stride>>=1u){if(lane<stride){sums[lane]+=sums[lane+stride];}workgroupBarrier();}
  let inverse_norm=inverseSqrt(sums[0]);
  for(var d=lane;d<params.width;d+=256u){output[batch*params.width+d]*=inverse_norm;}
}
`;

export class MeanPoolKernel {
  readonly pipeline:GPUComputePipeline;
  constructor(readonly device:GPUDevice){this.pipeline=device.createComputePipeline({layout:'auto',compute:{module:device.createShaderModule({code:meanPoolShader}),entryPoint:'main'},label:'mean + L2 pool'});}
  createRun(input:GPUBuffer,lengths:GPUBuffer,output:GPUBuffer,batch:number,sequence:number,width=2048):EncodableRun{
    const params=createBufferWithData(this.device,new Uint32Array([batch,sequence,width,0]),GPUBufferUsage.UNIFORM);
    return {pipeline:this.pipeline,bindGroup:this.device.createBindGroup({layout:this.pipeline.getBindGroupLayout(0),entries:[
      {binding:0,resource:{buffer:input}},{binding:1,resource:{buffer:lengths}},{binding:2,resource:{buffer:output}},{binding:3,resource:{buffer:params}},
    ]}),dispatch:[batch]};
  }
}
