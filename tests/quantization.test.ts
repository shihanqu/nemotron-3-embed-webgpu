...
import { dequantizeQ40Block, dequantizeQ4KBlock, dequantizeQ6KBlock, halfToFloat, quantizeQ40Block, requantizeMatrixToQ40 } from '../src/gguf/quantization.ts';
...
describe('GGML K-quant decoding', () => {
  ...
  it('quantizes and decodes Q4_0 with bounded error', () => {
    const values = Float32Array.from({ length: 32 }, (_, index) => (index - 16) / 4);
    const encoded = quantizeQ40Block(values);
    const decoded = dequantizeQ40Block(encoded);
    expect(Math.max(...decoded.map((value, index) => Math.abs(value - values[index])))).toBeLessThan(0.55);
  });

  it('requantizes Q6_K rows into consecutive Q4_0 blocks', () => {
    const source = new Uint8Array(210);
    source.fill(0, 0, 192);
    source.fill(1, 192, 208);
    new DataView(source.buffer).setUint16(208, 0x3c00, true);
    const converted = requantizeMatrixToQ40(source, 14, 256, 1);
    expect(converted.byteLength).toBe(8 * 18);
    expect(Array.from(dequantizeQ40Block(converted)).every((value) => value === -32)).toBe(true);
  });
});
